...
if __name__ == '__main__':
    ...
    # 加载模型
    model = YOLO("ultralytics/cfg/models/11/yolo11s-pose.yaml")  # 从头开始构建新模型
    # model = YOLO("runs/detect/v5n9/weights/best.pt")  # 加载预训练模型（推荐用于训练）
    # Use the model
    results = model.train(data="ultralytics/cfg/datasets/hand-keypoints.yaml",
                          epochs=300, batch=32, imgsz=256, workers=6, name=Path(model.cfg).stem)  # 训练模型
