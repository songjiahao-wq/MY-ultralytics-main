...
class ACBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1,
        ...
        self.deploy = deploy
        if deploy:
            self.fused_conv = nn.Conv2d(in_channels=in_channels, out_channels=out_channels, kernel_size=(
    ...
    # 将卷积层和BN层融合成一个等效的卷积核和偏置。
    def _fuse_bn_tensor(self, conv, bn):
        std = (bn.running_var + bn.eps).sqrt()  # 计算标准差
        t = (bn.weight / std).reshape(-1, 1, 1, 1)  # 计算缩放因子
        fused_kernel = conv.weight * t  # 将缩放因子应用到卷积核上
        fused_bias = bn.bias - bn.running_mean * bn.weight / std  # 计算融合后的偏置
        return fused_kernel, fused_bias

    # 将非对称卷积核添加到方形卷积核中。
    def _add_to_square_kernel(self, square_kernel, asym_kernel):
        asym_h = asym_kernel.size(2)  # 获取非对称卷积核的高度
        asym_w = asym_kernel.size(3)  # 获取非对称卷积核的宽度
        square_h = square_kernel.size(2)  # 获取方形卷积核的高度
        square_w = square_kernel.size(3)  # 获取方形卷积核的宽度
        # 将非对称卷积核添加到方形卷积核的中心位置
        square_kernel[:, :, square_h // 2 - asym_h // 2: square_h // 2 - asym_h // 2 + asym_h,
        square_w // 2 - asym_w // 2: square_w // 2 - asym_w // 2 + asym_w] += asym_kernel

    def get_equivalent_kernel_bias(self):
        hor_k, hor_b = self._fuse_bn_tensor(self.hor_conv, self.hor_bn)  # 融合水平卷积核和BN层
        ver_k, ver_b = self._fuse_bn_tensor(self.ver_conv, self.ver_bn)  # 融合垂直卷积核和BN层
        square_k, square_b = self._fuse_bn_tensor(self.square_conv, self.square_bn)  # 融合方形卷积核和BN层
        self._add_to_square_kernel(square_k, hor_k)  # 将水平卷积核添加到方形卷积核中
        self._add_to_square_kernel(square_k, ver_k)  # 将垂直卷积核添加到方形卷积核中
        return square_k, hor_b + ver_b + square_b  # 返回最终的等效卷积核和偏置

    # 将模块切换到部署模式，计算等效的卷积核和偏置，并删除训练时使用的卷积层和BN层，只保留一个融合后的卷积层 fused_conv。
