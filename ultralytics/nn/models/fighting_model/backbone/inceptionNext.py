...
def inceptionnext_tiny(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        state_dict = torch.load(r'C:\Users\Administrator\.cache\torch\hub\checkpoints\inceptionnext_tiny.pt')  # 加载本地权重文件
...
class InceptionNext_small(nn.Module):
    def __init__(self, c2, Layers=0 ):
        super().__init__()
        models = inceptionnext_tiny(pretrained=True)
...
def count_flops(model, input_tensor):
    ...
    def conv_hook(module, input, output):
        ...
        flops_per_instance = kernel_size * in_channels * out_channels
        num_instances = output.size(2) * output.size(3)
...
if __name__ == '__main__':
    ...
    input=torch.randn(1,96,160,160)
    ...
    for i in range(4):
        ...
        # flops = count_flops(moduless, input)
        # gflops = flops / 1e9
