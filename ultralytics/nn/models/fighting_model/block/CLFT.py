...
"""
卷积线性融合Transformer模块（CLFT）：基于Transformer结构的CLFT模块，设计了一种新颖的注意力机制，能够有效地增强目标特征并抑制噪声。
这一模块结合了卷积神经网络的局部特征提取能力与Transformer的全局建模能力，提升了红外小目标的检测效果。
"""
...
class CLFT(nn.Module):
    def __init__(self, in_dim, out_dim, in_feature=1, out_feature=1):
...
if __name__ == '__main__':
    block = CLFT(512, 512, 400, 20)  # 输入通道数，输出通道数 图像大小 H*W，H or W

    input = torch.randn(1, 512, 20, 20)  # 输入tensor形状 B C H W
